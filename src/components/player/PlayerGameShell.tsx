...
export function PlayerGameShell() {
  ...
  if (phase === 'NEEDS_GENESIS') return <PlayerGenesis busy={busy} error={error} onCreate={(userVision) => mutate(async () => { await playerApi.createWorld({ userVision }); })} />;
...
function PlayerGenesis({ busy, error, onCreate }: { busy: boolean; error: string | null; onCreate: (vision: string) => void }) { const [vision, setVision] = useState(''); return <main className="grid min-h-screen place-items-center bg-slate-950 p-4 text-slate-100"><form className="w-full max-w-xl space-y-4 rounded-xl border border-slate-800 bg-slate-900 p-6" onSubmit={(event) => { event.preventDefault(); onCreate(vision); }}><h1 className="text-xl font-black text-amber-300">Create your world</h1><p className="text-sm text-slate-300">Describe the world you want to explore. The world will establish its own history, rules, and people.</p><textarea className="min-h-40 w-full rounded bg-slate-950 p-3" value={vision} onChange={(event) => setVision(event.target.value)} minLength={20} disabled={busy} />{error && <p role="alert" className="rounded bg-rose-900 px-3 py-2 text-sm">{error}</p>}<button disabled={busy || vision.trim().length < 20} className="rounded bg-amber-400 px-4 py-2 font-bold text-slate-950 disabled:opacity-50">{busy ? 'Creating…' : 'Create world'}</button></form></main>; }
