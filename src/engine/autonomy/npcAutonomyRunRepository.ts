...
export class NpcAutonomyRunRepository {
  static async claimRun(input: Omit<NpcAutonomyRun, 'id' | 'status' | 'createdAt' | 'updatedAt'>): Promise<NpcAutonomyRun | null> {
    ...
    try { await dbManager.run('INSERT INTO npc_autonomy_runs (id, world_id, npc_id, epoch, trigger_reason, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [run.id, run.worldId, run.npcId, run.epoch, run.triggerReason, run.status, now, now]); return run; }
    catch (error) { if (/UNIQUE constraint failed/i.test(error instanceof Error ? error.message : String(error))) return null; throw error; }
  }
  ...
  static async listNonTerminalRuns(worldId: string): Promise<NpcAutonomyRun[]> { return (await dbManager.all<any>("SELECT * FROM npc_autonomy_runs WHERE world_id = ? AND status IN ('CLAIMED', 'DECIDED') ORDER BY epoch ASC, created_at ASC", [worldId])).map(this.map); }
